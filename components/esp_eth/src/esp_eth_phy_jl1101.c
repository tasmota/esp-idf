...
static esp_err_t jl1101_page_select(phy_jl1101_t *jl1101, uint32_t page)
{
    ...
    ESP_GOTO_ON_ERROR(eth->phy_reg_write(eth, jl1101->addr, ETH_PHY_PSR_REG_ADDR, psr.val), err, TAG, "write PSR failed");
    return ESP_OK;
err:
    return ret;
}
...
static esp_err_t jl1101_update_link_duplex_speed(phy_jl1101_t *jl1101)
{
    ...
    ESP_GOTO_ON_ERROR(eth->phy_reg_read(eth, jl1101->addr, ETH_PHY_BMSR_REG_ADDR, &(bmsr.val)), err, TAG, "read BMSR failed");
    ESP_GOTO_ON_ERROR(eth->phy_reg_read(eth, jl1101->addr, ETH_PHY_ANLPAR_REG_ADDR, &(anlpar.val)), err, TAG, "read ANLPAR failed");
    eth_link_t link = bmsr.link_status ? ETH_LINK_UP : ETH_LINK_DOWN;
    /* check if link status changed */
    if (jl1101->link_status != link) {
        /* when link up, read negotiation result */
        if (link == ETH_LINK_UP) {
            ESP_GOTO_ON_ERROR(eth->phy_reg_read(eth, jl1101->addr, ETH_PHY_BMCR_REG_ADDR, &(bmcr.val)), err, TAG, "read BMCR failed");
...
static esp_err_t jl1101_reset(esp_eth_phy_t *phy)
{
    ...
    ESP_GOTO_ON_ERROR(eth->phy_reg_write(eth, jl1101->addr, ETH_PHY_BMCR_REG_ADDR, bmcr.val), err, TAG, "write BMCR failed");
    ...
    ESP_GOTO_ON_FALSE(to < jl1101->reset_timeout_ms / 10, ESP_FAIL, err, TAG, "reset timeout");
    return ESP_OK;
err:
    return ret;
}
 ...
 *       Instead, the negotiation result is fetched by linker timer, see `jl1101_get_link()`
 */
...
static esp_err_t jl1101_pwrctl(esp_eth_phy_t *phy, bool enable)
{
    ...
    if (!enable) {
        ...
        ESP_GOTO_ON_FALSE(to < jl1101->reset_timeout_ms / 10, ESP_FAIL, err, TAG, "power up timeout");
    }
    return ESP_OK;
err:
    return ret;
...
static esp_err_t jl1101_advertise_pause_ability(esp_eth_phy_t *phy, uint32_t ability)
{
    ...
    anar_reg_t anar;
    ESP_GOTO_ON_ERROR(eth->phy_reg_read(eth, jl1101->addr, ETH_PHY_ANAR_REG_ADDR, &(anar.val)), err, TAG, "read ANAR failed");
    if (ability) {
    ...
    ESP_GOTO_ON_ERROR(eth->phy_reg_write(eth, jl1101->addr, ETH_PHY_ANAR_REG_ADDR, anar.val), err, TAG, "write ANAR failed");
    return ESP_OK;
err:
    return ret;
}
...
static esp_err_t jl1101_init(esp_eth_phy_t *phy)
{
    ...
    if (jl1101->addr == ESP_ETH_PHY_ADDR_AUTO) {
        ESP_GOTO_ON_ERROR(esp_eth_detect_phy_addr(eth, &jl1101->addr), err, TAG, "Detect PHY address failed");
    }
    /* Power on Ethernet PHY */
    ESP_GOTO_ON_ERROR(jl1101_pwrctl(phy, true), err, TAG, "power control failed");
    /* Reset Ethernet PHY */
    ESP_GOTO_ON_ERROR(jl1101_reset(phy), err, TAG, "reset failed");
    /* Check PHY ID */
    phyidr1_reg_t id1;
    phyidr2_reg_t id2;
    ESP_GOTO_ON_ERROR(eth->phy_reg_read(eth, jl1101->addr, ETH_PHY_IDR1_REG_ADDR, &(id1.val)), err, TAG, "read ID1 failed");
    ESP_GOTO_ON_ERROR(eth->phy_reg_read(eth, jl1101->addr, ETH_PHY_IDR2_REG_ADDR, &(id2.val)), err, TAG, "read ID2 failed");
    ESP_GOTO_ON_FALSE(id1.oui_msb == 0x937C && id2.oui_lsb == 0x10 && id2.vendor_model == 0x2, ESP_FAIL, err, TAG, "wrong chip ID");
    return ESP_OK;
err:
    return ret;
}
...
static esp_err_t jl1101_deinit(esp_eth_phy_t *phy)
{
    ...
    ESP_GOTO_ON_ERROR(jl1101_pwrctl(phy, false), err, TAG, "power control failed");
    return ESP_OK;
err:
    return ret;
}
...
esp_eth_phy_t *esp_eth_phy_new_jl1101(const eth_phy_config_t *config)
{
    ...
    jl1101->parent.loopback = jl1101_loopback;
